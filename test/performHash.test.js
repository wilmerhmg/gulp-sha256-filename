...
describe('performHash.js', () => {
	...
	it('should handle `{name}-{hash}{ext}`', () => {
		expect(posixPath(performHash('{name}-{hash}{ext}', file).path)).to.equal('/dog/cat/myFile-2e99758548972a8e8822ad47fa1017ff72f06f3ff6a016851f45c398732bc50c.txt');
	});

	it('should handle `{name:3}-{hash:8}{ext:2}`', () => {
		expect(posixPath(performHash('{name:3}-{hash:8}{ext:2}', file).path)).to.equal('/dog/cat/myF-2e997585.t');
	});
